// pages/api/analytics/devices.ts
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!credentialsJson) {
    ...
    return res.status(500).json({ error: 'Учетные данные для GA4 API не настроены' });
  }
  ...
  try {
    ...
    const dimensions = [{ name: 'deviceCategory' }, { name: 'operatingSystem' }];
    ...
    console.error('Ошибка в API-маршруте /api/analytics/devices:', error);
    ...
    res.status(500).json({
      error: 'Не удалось получить данные об устройствах',
