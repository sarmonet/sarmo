// pages/api/analytics/traffic-sources.ts
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
	...
	try {
		...
		const dimensions = [{ name: 'defaultChannelGroup' }]
		...
		console.error(
			'Ошибка в API-маршруте /api/analytics/traffic-sources:',
			error
		)
		...
		res.status(500).json({
			error: 'Не удалось получить данные об источниках трафика',
